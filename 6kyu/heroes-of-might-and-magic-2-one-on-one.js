...
function whoWouldWin(mon1, mon2) {
  while (mon1.number > 0 && mon2.number > 0) {
    mon2.number = (calcTotalHP(mon2) - calcDmg(mon1)) / mon2.hitpoints;
    if (mon2.number < 0) break;
    mon1.number = (calcTotalHP(mon1) - calcDmg(mon2)) / mon1.hitpoints;
  }
  return mon1.number > 0 ? winMsg(mon1) : winMsg(mon2);
}

function calcTotalHP(mon) {
  return mon.hitpoints * mon.number;
}

function calcDmg(mon) {
  return Math.ceil(mon.number) * mon.damage;
}

function winMsg(winner) {
  return `${Math.ceil(winner.number)} ${winner.type}(s) won`;
}
